...
def export_pdf(filename, extension, default_dpi, savefile=False):
    ...
    pdf.setCreator('Khayrallah Center')
    pdf.setTitle(os.path.basename(filename))
    dpi = default_dpi
    # for image in images:
    im = Image.open(os.path.join(settings.FILE_STORE,filename+extension))
    w, h = im.size
    try:
        dpi = im.info['dpi'][0]
    except KeyError:
    ...
    pdf.drawImage(image_file, 0, 0, width=width, height=height)
    add_text_layer(pdf, filename, height, dpi)
    pdf.showPage()
    pdf.save()
...
def add_text_layer(pdf, image, height, dpi):
    ...
    hocrfile=os.path.join(settings.HOCR_STORE,image+".hocr")
...
def HOCR2PDF(img_name):

#     # SAVE IMAGE AND CORRESPONDING HOCR IN SAME PATH
#     # GET FULL PATH OF IMAGE FILE
#     # SAVES PDF IN SAME DIRECTORY

    # filename=os.path.splitext(img_name)[0]
    hocr_file=os.path.join(settings.HOCR_STORE,img_name+".hocr")
    pdf_file=os.path.join(settings.PDF_STORE,img_name+".pdf")
    if not os.path.exists(hocr_file):
        print("No HOCR file found")
        return
    file_ext=[]
    isthere=False
    # for ext in [".jpg",".jpeg",".png",".gif"]:
    ext = '.png'
    if os.path.exists(os.path.join(settings.FILE_STORE,img_name+ext)):
            export_pdf(img_name, ext, 300, pdf_file)
            isthere=True
    if not isthere:
        print("No Image file found")
        return 
    # export_pdf("backup_input",300,pdf_file)
