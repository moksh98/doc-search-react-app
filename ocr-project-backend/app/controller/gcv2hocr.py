...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="$lang" lang="$lang">
  ...
  <body>
    ...
    """),
        ...
        'ocrx_word': Template("""
                <span class='ocrx_word' id='$htmlid' lang='$lang' title='bbox $x0 $y0 $x1 $y1'>$content</span>"""),
        'ocr_carea':Template("""
          ...
          for block_id, block_json in enumerate(page_json['blocks']):
            ...
            for paragraph_id, paragraph_json in enumerate(block_json['paragraphs']):
                ...
                for word_id, word_json in enumerate(paragraph_json['words']):
                    ...
                    language="unknown"
                    if "property" in word_json:
                        language=word_json["property"]["detectedLanguages"][0]["languageCode"]
                    word = GCVAnnotation(ocr_class='ocrx_word', htmlid="word_"+ str(block_id) + "_" + str(paragraph_id)+"_"+ str(word_id), lang=language, content=word_text, box=box)
...
def GCV2HOCR(json_string, hocr_filename, **kargs):
    resp = json.loads(json_string)
    resp = resp["responses"][0] if "responses" in resp and len(resp["responses"]) >= 0 and "textAnnotations" in resp["responses"][0] else False

    page = fromResponse(resp, hocr_filename, **kargs)

    # if kargs.get("savefile"):
